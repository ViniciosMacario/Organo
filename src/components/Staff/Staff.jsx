...
function Staff({responseData, handleStateTimeForm}){
  ...
  return (
    <Container style={{background: `${hexToRgba(responseData.color, 0.8)}`}}>
      ...
      <Members statusColaborador={responseData.colaboradores.length === 0}>
        {responseData.colaboradores.length === 0 
          ...
          : 
            responseData.colaboradores.map(data => { return (<Card key={data.id} nome={data.nome} cargo={data.cargo} color={responseData.color}/>)})
        }        
      </Members>
    </Container>
  )
